...
const timerChecks_empty = document.querySelectorAll(
  ".timer_check1, .timer_check2",
);

const timerChecks_checked = document.querySelectorAll(
  ".timer_check21, .timer_check22",
);
...
function handleClickCreate(event) {
  ...
  const countdown_time =
    (parseInt(countdown[0].value) * 60 + parseInt(countdown[1].value)) * 1000;
  ...
  if (dropgen1_1 === false) {
    ...
    function selectColour(drop) {
      ...
      if (drop == drop1) {
        ...
        dropdowns1.value = drop11.innerText;
      } else if (drop == drop2) {
        check2.style.display = "block";
        dropdowns1.value = drop21.innerText;
      } else if (drop == drop3) {
        check3.style.display = "block";
        dropdowns1.value = drop31.innerText;
      }
...
document.addEventListener("click", function (event) {
  if (dropgen1_1 === true && opendrop === true) {
    dropgen1_1 = false;
  } else {
...
const check1_2 = document.querySelector(
  ".dropdown_2 > .drop_1 > .fa-check.d21",
);
const check2_2 = document.querySelector(
  ".dropdown_2 > .drop_2 > .fa-check.d22",
);
const check3_2 = document.querySelector(
  ".dropdown_2 > .drop_3 > .fa-check.d23",
);
...
dropdowns2.addEventListener("click", function () {
  if (dropgen1_2 === false) {
    ...
    function selectColour(drop) {
      ...
      if (drop == drop1_2) {
        check1_2.style.display = "block";
        dropdowns2.value = drop11_2.innerText;
      } else if (drop == drop2_2) {
        check2_2.style.display = "block";
        dropdowns2.value = drop21_2.innerText;
      } else if (drop == drop3_2) {
        check3_2.style.display = "block";
        dropdowns2.value = drop31_2.innerText;
      }
...
document.addEventListener("click", function (event) {
  if (dropgen1_2 === true && opendrop_2 === true) {
    dropgen1_2 = false;
  } else {
...
function chess_game(
  ...
  list_of_options[0].addEventListener("click", function () {
    socket.send(
      JSON.stringify({
        type: "options",
        payload: { id: id, type: "save_and_resume" },
      }),
    );
  });
  ...
  list_of_options[1].addEventListener("click", function () {
    socket.send(
      JSON.stringify({ type: "options", payload: { id: id, type: "draw" } }),
    );
  });
  ...
  list_of_options[2].addEventListener("click", function () {
    socket.send(
      JSON.stringify({
        type: "options",
        payload: { id: id, type: "resign", winner: p2, looser: p1 },
      }),
    );
  ...
  socket.onmessage = function (event) {
    ...
    if (message.type === "move") {
      if (payload.moveA === 101) {
        document.querySelector(".user1 h6").style.display = "block";
        return;
      }
      ...
      if (payload.castling === "Yes") {
        if (payload.moveA < payload.moveB) {
          if (ronda === 2) {
            moveBlack(88, 86);
          } else {
            moveBlack(88, 85);
      ...
      if (payload.pawn_promotion !== null) {
        ...
        blackPieces = blackPieces.filter(
          (piece) => piece.position != 99 - payload.moveB,
        );

        if (payload.pawn_promotion == "Queen") {
          promo = new Queen(color2, 99 - payload.moveB);
        } else if (payload.pawn_promotion == "Knight") {
          promo = new Knight(color2, 99 - payload.moveB);
        }
      ...
      if (
        ChessPiece.check(
          whitePieces.filter((piece) => piece instanceof King)[0].position,
        )
      ) {
    ...
    if (message.type === "end_game") {
      container.removeEventListener("mousedown", EventHear);
      document.querySelector(".time_form .ID_letters h5").innerText =
        payload.winner;
    ...
    if (message.type === "resign") {
      ...
      resign_form_1.querySelector(".game_created h1").innerText =
        `${payload.looser} has resigned.`;
  ...
  function moveBlack(A, B) {
    ...
    if (whitePieces.map((i) => i.position).includes(perspectiveB)) {
      ...
      whitePieces = whitePieces.filter(
        (piece) => piece.position !== perspectiveB,
      );
    } else {
      let image = element.querySelector("img");
      element.removeChild(image);
    ...
    blackPieces
      .filter((piece) => piece.position == perspectiveA)
      .forEach((piece) => (piece.position = perspectiveB));
  ...
  class ChessPiece {
    ...
    static isOccupied(position) {
      ...
      return (
        whitePieces.map((i) => i.position).includes(position) ||
        blackPieces.map((i) => i.position).includes(position)
      ); // Fixed typo 'isOcuppied' to 'isOccupied'
    }
    ...
    static check(place) {
      const getLinearMov_Y = () => {
        ...
        for (let s of [1, -1]) {
          for (let j of horizontalMovementsY) {
            ...
            if (ChessPiece.isOccupied(newPosition)) {
              if (
                whitePieces.filter((i) => i.position === newPosition).length > 0
              ) {
                return false;
              }

              const piece = blackPieces.filter(
                (i) => i.position == newPosition,
              )[0];

              if (
                piece instanceof Queen ||
                piece instanceof Rook ||
                (j == 1 && piece instanceof King)
              ) {
                return true;
              }
          ...
          for (let j of diagMovements_xy) {
            ...
            if (ChessPiece.isOccupied(newPosition)) {
              ...
              if (
                ...
                (j == 11 && piece instanceof King) ||
                (j == 11 && s == -1 && piece instanceof Pawn)
              ) {
                return true;
              }
          ...
          for (let j of diagMovements_mxy) {
            ...
            if (ChessPiece.isOccupied(newPosition)) {
              ...
              if (
                piece instanceof Queen ||
                piece instanceof Bishop ||
                (j == 9 && piece instanceof King) ||
                (j == 9 && s == -1 && piece instanceof Pawn)
              ) {
                return true;
              }
            }
      ...
      const horsey_Mov = () => {
        ...
        for (let move of knightMoves) {
          ...
          if (
            ChessPiece.isValidBoundary(newPosition) &&
            blackPieces.map((i) => i.position).includes(newPosition)
          ) {
            const piece = blackPieces.filter(
              (i) => i.position == newPosition,
            )[0];
    ...
    static mate(place) {
      for (let piece of whitePieces) {
        for (let j of piece.canMoveTo()) {
          ...
          piece.position = j;
          const king_position = whitePieces.filter(
  ...
  class Pawn extends ChessPiece {
    ...
    canMoveTo() {
      ...
      for (let j of [10]) {
        if (
          ChessPiece.isValidBoundary(this.position - j) &&
          !ChessPiece.isOccupied(this.position - j)
        ) {
      ...
      for (let j of [11, 9]) {
        if (
          ChessPiece.isValidBoundary(this.position - j) &&
          blackPieces.map((i) => i.position).includes(this.position - j)
        ) {
          valid_moves.push(this.position - j);
        }
      }
  ...
  class Knight extends ChessPiece {
    ...
    canMoveTo() {
      ...
      for (let move of knightMoves) {
        ...
        if (
          ChessPiece.isValidBoundary(newPosition) &&
          !whitePieces.map((i) => i.position).includes(newPosition)
        ) {
  ...
  class Queen extends ChessPiece {
    ...
    canMoveTo() {
      return this.getLinearMov_X()
        .concat(this.getLinearMov_Y())
        .concat(this.getDiagMov_xy())
        .concat(this.getDiagMov_mxy());
    }
  }
          ...
          .forEach((i) => {
            if (i.castling === true) {
              ...
              if (this.position < i.position) {
                while (this.position + k < 88) {
                  if (
                    whitePieces
                      .concat(blackPieces)
                      .map((i) => i.position)
                      .includes(this.position + k)
                  ) {
                    posible = false;
                    break;
                  }
                ...
                if (posible === true) {
                  while (this.position + j <= 88) {
                    if (ChessPiece.check(this.position + j)) {
                      posible = false;
                      break;
                    }
                    j += 1;
                ...
                if (posible === true) {
                  validMoves.push(this.position + 2);
                }
              } else {
                while (this.position - k > 81) {
                  if (
                    whitePieces
                      .concat(blackPieces)
                      .map((i) => i.position)
    ...
    if (selected === 0) {
      ...
      if (currentPiece.canMoveTo().includes(pos)) {
        ...
        if (blackPieces.map((i) => i.position).includes(pos)) {
          let recover_if_check = blackPieces.filter(
        ...
        if (currentPiece instanceof Rook || currentPiece instanceof King) {
          currentPiece.castling = false;
        }

        if (
          currentPiece instanceof King &&
          (pos === posSelected + 2 || pos === posSelected - 2)
        ) {
          castle = "Yes";

          if (pos > posSelected) {
            castle_tower = whitePieces.filter(
              (piece) => piece instanceof Rook && piece.position > posSelected,
            )[0];

            const gridItem_tower = document.getElementById(
              String(castle_tower.position),
            );
            const gridItem_now_tower = document.getElementById(
              String(posSelected + 1),
            );
        ...
        socket.send(
          JSON.stringify({
            type: "move",
            payload: { id, player, moveA, moveB, turn, pawn_promotion, castle },
          }),
        );
        clearInterval(timerId);
